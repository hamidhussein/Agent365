    ...
    <div className="flex flex-col h-[600px] bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
      ...
      <div className="px-6 py-4 border-b border-border bg-secondary/30 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className={`w-8 h-8 rounded-lg ${draftAgent.color || 'bg-muted'} flex items-center justify-center text-white`}>
            <Bot size={18} />
          </div>
          <div>
            <h3 className="text-sm font-bold text-foreground">{draftAgent.name || 'Agent Preview'}</h3>
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider font-semibold">Live Testing</p>
          </div>
        </div>
      ...
      {messages.length === 0 && (
        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-muted/10">
          ...
          <h4 className="text-foreground font-bold mb-2">Test your agent</h4>
          <p className="text-sm text-muted-foreground max-w-[200px]">Send a message to see how your agent responds with the current configuration.</p>
        </div>
      )}

      {/* Messages */}
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-6 scroll-smooth bg-muted/5">
        {messages.map((m, i) => (
          <div key={i} className={`flex gap-4 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
            <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${m.role === 'model' ? (draftAgent.color || 'bg-muted') : 'bg-secondary text-muted-foreground'
                  ...
                  components={{
                    a: ({ node, ...props }) => {
                      const href = props.href?.startsWith('/api/files/')
                        ? `${API_BASE}${props.href}`
                        : props.href;
                      return <a {...props} href={href} className="text-blue-500 hover:underline font-bold" target="_blank" rel="noopener noreferrer" />;
                    },
      ...
      <div className="p-4 bg-card border-t border-border">
        ...
        >
          <input
            ...
            className="w-full bg-secondary/50 border border-border rounded-xl px-4 py-3 pr-12 text-sm text-foreground placeholder-muted-foreground focus:outline-none focus:border-primary transition-all"
          />
          <button
            type="submit"
            disabled={!input.trim() || isLoading}
            className="absolute right-2 top-2 w-8 h-8 bg-secondary hover:bg-primary hover:text-white text-muted-foreground rounded-lg flex items-center justify-center transition-colors disabled:opacity-50"
          >
            <Send size={16} />
          </button>
        </form>
