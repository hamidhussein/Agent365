...
export const ChatInterface = ({
  ...
  return (
    <div className="flex h-screen bg-background relative overflow-hidden text-foreground">
      ...
      {/* Sidebar Info */}
      <div className="w-80 bg-muted/30 backdrop-blur-xl border-r border-border flex flex-col hidden lg:flex relative z-10">
            ...
            >
              ...
              <AnimatePresence initial={false}>
                {messages.map((msg, index) => (
                  ...
                  >
                          ...
                          <ReactMarkdown
                            ...
                            components={{
                              code({ className, children, ...props }) {
                                ...
                                if (!match) {
                                  return <code className="bg-secondary px-1.5 py-0.5 rounded text-primary font-mono text-[0.9em]" {...props}>{code}</code>;
                                }
                                return (
                                  <div className="relative group/code my-4 not-prose">
                                    <div className="absolute -top-3 left-4 px-2.5 py-1 bg-secondary border border-border rounded-lg text-[10px] text-muted-foreground font-mono z-10 shadow-sm">
                                      {match[1].toUpperCase()}
                                    </div>
                                    <button
                                      onClick={() => copyToClipboard(code)}
                                      className="absolute right-3 top-3 p-2.5 text-muted-foreground hover:text-foreground bg-secondary/80 rounded-lg opacity-0 group-hover/code:opacity-100 transition-all border border-border backdrop-blur-sm hover:bg-secondary"
                                      title="Copy code"
                                    >
                                      <Copy size={14} />
                                    </button>
                                    <pre className="m-0 bg-secondary/50 border border-border rounded-2xl p-5 overflow-x-auto scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent">
                    ...
                    {msg.role === 'user' && (
                      <div className="flex-shrink-0 mt-1">
                        <div className="w-8 h-8 rounded-xl bg-primary flex items-center justify-center text-primary-foreground shadow-sm font-semibold text-sm">
                          U
                        </div>
                      </div>
                    )}
              ...
              {isThinking && (
                ...
                >
                  {/* Bot Avatar */}
                  <div className="flex-shrink-0 mt-1">
                    <div className={`w-8 h-8 rounded-xl ${agent.color} flex items-center justify-center text-primary-foreground shadow-sm`}>
                      {agentModelOption?.icon || <Bot size={16} />}
                    </div>
                  </div>

                  <div className="bg-card border border-border rounded-2xl rounded-bl-md px-6 py-4 shadow-sm">
                ...
                <div className="flex items-center justify-between gap-2 mb-3 px-2">
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-semibold text-muted-foreground hover:text-foreground hover:bg-secondary disabled:opacity-30 disabled:cursor-not-allowed transition-all border border-border hover:border-border/80 backdrop-blur-sm"
                    ...
                    {agent.allow_reviews && lastExecutionId && !isThinking && (
                      <button
                        ...
                        className={`flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-semibold transition-all border backdrop-blur-sm ${reviewStatus === 'pending'
                          ? 'text-amber-400 border-amber-400/40 bg-amber-400/10 cursor-not-allowed'
                          : 'text-muted-foreground border-border hover:text-foreground hover:bg-secondary hover:border-border/80'
                          }`}
                ...
                <div className="bg-background border border-border rounded-3xl p-1.5 shadow-lg relative group/input transition-all">
                  ...
                  {attachment && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, y: -10 }}
                      className="mb-2 mx-2 flex items-center justify-between p-3 bg-card border border-border rounded-2xl backdrop-blur-sm shadow-sm"
                    >
                      <div className="flex items-center gap-3 overflow-hidden">
                        ...
                        <div className="flex flex-col overflow-hidden">
                          <span className="text-sm text-foreground truncate font-medium max-w-[250px]">{attachment.name}</span>
                          <span className="text-xs text-muted-foreground">Attached as context</span>
                        </div>
                      </div>
                      <button
                  ...
                  <div className="relative flex items-end gap-2 px-2">
                    {agent.enabledCapabilities?.fileHandling && (
                      <>
                        ...
                        <Button
                          ...
                          variant="ghost"
                          className="mb-1 rounded-xl w-11 h-11 p-0 flex items-center justify-center shrink-0 text-muted-foreground hover:text-foreground hover:bg-secondary transition-all disabled:opacity-30"
                          title="Attach file"
                        >
                    ...
                    <Button
                      ...
                      className={`mb-1 rounded-2xl w-12 h-12 p-0 flex items-center justify-center shrink-0 shadow-2xl transition-all ${inputValue.trim() && !isThinking
