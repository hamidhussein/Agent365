...
const ReviewsPage: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'pending':
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20 rounded-full text-xs font-medium">
      ...
      case 'completed':
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-green-500/10 text-green-600 dark:text-green-400 border border-green-500/20 rounded-full text-xs font-medium">
  ...
  return (
    <div className="container mx-auto max-w-6xl px-4 py-8">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-foreground mb-2">Expert Reviews</h1>
        <p className="text-muted-foreground">Manage review requests from users of your agents</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 bg-card/60 rounded-lg border border-border">
        ...
        <div className="space-y-4">
          {reviews.map((review: Review) => (
            <div
              key={review.id}
              className="bg-card/80 border border-border rounded-lg p-6 hover:border-border/80 transition-colors"
            >
              {/* Header */}
