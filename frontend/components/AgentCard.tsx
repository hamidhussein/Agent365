...
const AgentCard: React.FC<AgentCardProps> = ({ agent, onSelect, onSelectCreator, isFavorited, onToggleFavorite }) => {
    ...
    return (
        <div onClick={() => onSelect(agent.id)} className="group relative overflow-hidden rounded-lg border border-border bg-card transition-all duration-300 hover:-translate-y-1 hover:border-primary hover:shadow-lg hover:shadow-primary/10 text-left cursor-pointer">
            <div className="relative">
                <img src={agent.imageUrl} alt={agent.name} className="h-48 w-full object-cover" />
                <button
                    onClick={handleFavoriteClick}
                    className={`absolute top-3 right-3 z-10 flex h-8 w-8 items-center justify-center rounded-full bg-background/50 backdrop-blur-sm transition-colors duration-200 border border-white/20 ${isFavorited ? 'text-red-500 bg-white shadow-sm' : 'text-foreground/70 hover:text-foreground hover:bg-white'} `}
            ...
            <div className="p-5">
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {agent.tags.slice(0, 2).map(tag => (
                        <span key={tag} className="rounded-full bg-secondary px-2 py-0.5 text-xs font-medium text-secondary-foreground">{tag}</span>
                    ))}
                </div>
                <h3 className="mt-3 text-lg font-semibold text-foreground group-hover:text-primary transition-colors">{agent.name}</h3>
            ...
            <div className="flex items-center justify-between border-t border-border bg-secondary/30 p-4">
                ...
                <div className="flex items-center rounded-full bg-green-100 px-2.5 py-1 text-xs font-semibold text-green-700 dark:bg-green-900/30 dark:text-green-400">
