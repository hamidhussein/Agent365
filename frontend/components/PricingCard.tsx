...
const PricingCard: React.FC<PricingCardProps> = ({ agent, onRunAgent }) => {
    ...
    return (
        <div className="sticky top-24 rounded-2xl border border-border bg-card/80 p-6 shadow-xl shadow-black/5">
            <div className="flex items-baseline justify-center">
                <span className="text-5xl font-extrabold text-foreground">{agent.price}</span>
                <span className="ml-2 text-base font-semibold text-muted-foreground">credits/run</span>
            </div>

            <div className="mt-6 space-y-4">
                {showCreatorStudioNote && (
                    <p className="text-sm text-primary font-medium text-center bg-primary/5 p-3 rounded-xl border border-primary/20">
                        You own this Creator Studio agent. Open the studio to chat or edit.
                    </p>
                )}
                <button 
